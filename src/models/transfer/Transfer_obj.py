...
class Transfer_obj:
    ...
    def __init__(self, org_name):
        ...
        self.model = load_model(os.path.join(MODELS_OBJECTS_PATH,self.src_model_name))
    ...
    def retrain_model(self, t_size):
        if t_size !=0:
            ...
            self.model.fit([self.x_train, self.sequences], self.y, epochs=5)
    ...
    def eval_model(self,t_size):
        ...
        pred = self.model.predict([self.x_val, self.sequences_tst])
        date_time, model_name = create_evaluation_dict(self.src_model_name+"_"+t_size,self.dst_org_name,pred, self.y_val)
